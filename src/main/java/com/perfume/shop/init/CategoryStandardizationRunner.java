...
public class CategoryStandardizationRunner implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        try {
            ...
            if (oudUpdated > 0)
                log.info("✓ Updated {} products to OUD_RESERVE", oudUpdated);
            ...
            if (bakhoorUpdated > 0)
                log.info("✓ Updated {} products to BAKHOOR", bakhoorUpdated);
            ...
            if (chemicalUpdated > 0)
                log.info("✓ Updated {} products to AROMA_CHEMICALS", chemicalUpdated);

            // Standardize MEN, WOMEN, UNISEX
            jdbcTemplate
                    .update("UPDATE products SET category = 'MEN' WHERE UPPER(category) IN ('MEN', 'MENS', 'MALE')");
            jdbcTemplate.update(
